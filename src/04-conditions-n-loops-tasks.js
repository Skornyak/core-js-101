...
function isCreditCardNumber(ccn) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    if (!(i % 2)) {
      ...
      if (+arr[i] > 9) arr[i] = (+arr[i][0] + +arr[i][1]).toString();
    }
  }
  return !(arr.reduce((sum, ind) => +sum + +ind) % 10);
}
...
function getDigitalRoot(num) {
  ...
  if (result > 9) return +result.toString()[0] + +result.toString()[1];
  return result;
}
...
function isBracketsBalanced(str) {
  ...
  if (stack.length === 0 && f) result = true;
  return result;
}
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < arrPathes[0].length; i += 1) {
    const part = arrPathes[0][i];
    if (arrPathes.every((current) => current[i] === part)) {
      result.push(part);
    }
  }
...
function getMatrixProduct(m1, m2) {
  ...
  for (let i = 0; i < c2; i += 1) {
    for (let j = 0; j < r1; j += 1) {
      let x = 0;
